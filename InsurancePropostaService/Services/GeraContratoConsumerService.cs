...
{
    public class GeraContratoConsumerService : BackgroundService
    {
        private readonly IConnection _connection;
        private readonly IModel _channel;
        private readonly RabbitMQSettings _settings;
        private readonly IServiceProvider _serviceProvider;
        private readonly ILogger<GeraContratoConsumerService> _logger;

        public GeraContratoConsumerService(
            IOptions<RabbitMQSettings> settings,
            IServiceProvider serviceProvider,
            ILogger<GeraContratoConsumerService> logger)
        {
        ...
        protected override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.Received += async (sender, e) =>
            {
                ...
                {
                    _logger.LogError(ex, $"Error processing contract generation message (attempt {retryCount + 1}/{_settings.MaxRetries + 1})");
            ...
            _channel.BasicConsume(_settings.GeraContratoRequestQueue, false, consumer);
        ...
        private async Task ProcessMessage(BasicDeliverEventArgs e)
        {
            ...
            var request = JsonSerializer.Deserialize<GeraContratoRequestDto>(requestJson);

            if (request == null)
            {
                _logger.LogWarning("Received invalid contract generation request message");
                return;
            }

            _logger.LogInformation($"Processing contract generation request for proposta {request.PropostaId}");

            using var scope = _serviceProvider.CreateScope();
            var operacoesContratoUC = scope.ServiceProvider.GetRequiredService<IOperacoesContratoUC>();

            var response = new GeraContratoResponseDto
            {
            ...
            try
            {
                var contrato = await operacoesContratoUC.GeraContratoProposta(
                    request.PropostaId,
                    request.UserId,
                ...
                response.ContratoId = contrato.id; // Since the UC doesn't return an ID, we'll use 1 for success

                _logger.LogInformation($"Contract generated successfully for proposta {request.PropostaId}");
            }
            catch (Exception ex)
            {
                response.ErrorMessage = ex.Message;
                _logger.LogError(ex, $"Failed to generate contract for proposta {request.PropostaId}");
            }
            ...
            _logger.LogInformation($"Contract generation response sent for proposta {request.PropostaId}");
        }
        ...
        private async Task RequeueWithRetry(BasicDeliverEventArgs e, int retryCount, Exception lastException)
        {
            try
            {
                ...
                props.Headers = new Dictionary<string, object>
                {
                    ...
                    {"original-queue", _settings.GeraContratoRequestQueue},
